...
- 🔎 **Reliable subcommands for arbitrary words** — pron, syn, ant, and ex now select only their exact result category.
- 🗣️ **Dedicated pronunciation results** — Phonetic text and audio are presented independently from definitions.
- 📚 **Complete related-word data** — Synonyms and antonyms are collected from both meaning-level and definition-level API fields.
- ⚙️ **Explicit commands override display toggles** — A requested category is returned even when hidden from the default lookup.
- 💬 **Clear empty states** — The plugin explains when a word has no requested pronunciation, synonyms, antonyms, or examples.
- 🧪 **Regression coverage** — Tests cover every long and short alias with words beyond hello and happy.
